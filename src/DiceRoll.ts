...
class DiceRoll implements Readonly<HasNotation> {
   ...
   */
  constructor(notation: string|RollOutputData) {
    if (!notation) {
   ...
   */
  static import(data: string|RollOutputData): DiceRoll {
    if (!data) {
  ...
  #rollExpressions(engine?: Engine): ExpressionResult {
    ...
    const results = new ResultGroup(
      this
        ...
        // @todo figure out correct type rather than use `any`
        .filter((value) => !!value || (value === 0)) as any
    );
   ...
   * @throws {TypeError} Rolls must be a valid result object, or an array
   */
  #setRolls(rolls: ExpressionResult|ResultCollection|RollResult[]): void {
